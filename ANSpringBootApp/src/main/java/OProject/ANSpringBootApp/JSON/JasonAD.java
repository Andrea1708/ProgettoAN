...
public static ArrArrayList<E>tion> JSONParser(JSONObject json) {
	...
	if(json != null) {
		...
		if( alldata != null) {
			...
			for(int i=0; i< alldata.length; i++) {
				 ...
				 */
				try {
					JSONObject obj = (JSONObject) (alldata.getJSONObject(i).get("Confirmed"));
					COCA.setConfirmed(obj.getInt/** capire che ci va qui */);
				}catch (JSONException e) {
					COCA.setConfirmed(0);
				}
				try {
					JSONObject obj = (JSONObject) (alldata.getJSONObject(i).get("Deaths"));
					COCA.setDeaths(/** capire che ci va qui */);
				}catch (JSONException e) {
					COCA.setDeaths(0);
				}
				try {
